...
export function XpCalculator() {
  ...
  const filterProjects = () => {
    ...
    const filteredProjects = isLearner ? Projects.filter((project) => learnerProjects.includes(project.id)) : Projects;
  ...
  const calculate_end_level = (current_level: number, values: string | number, note: string | number, has_coa_bonus: boolean) => {
    ...
    const level_down_xp = level_xp[level_down].xp;

    const level_sup = Math.ceil(current_level);
    const level_sup_xp = level_xp[level_sup].xp;
    ...
    const parsedValues = parseInt(values.toString());
    ...
    const min_xp = level_xp[i - 1].xp;
    const max_xp = level_xp[i].xp;
