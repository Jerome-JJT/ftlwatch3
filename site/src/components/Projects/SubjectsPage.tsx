...
export function SubjectsPage(): JSX.Element {
  ...
  React.useEffect(() => {document.title = commonTitle('Subjects page');}, []);


  React.useEffect(() => {
    axios
      .get('/?page=projects&action=get_subjects',
          ...
          const displayValues = res.data.values.map((subject: any) => {
            res.data.columns.forEach((col: ColumnProps) => {
              if (col.field === 'details') {
                ...
                subject[col.field] = <p title={subject[col.field]} className="text-black truncate max-w-[100px]">{subject[col.field]}</p>;
              }
            });
